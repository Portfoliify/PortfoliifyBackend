...
for(const rp of repoPaths) {
  const rP = path.resolve(__dirname, rp);
    ...
    process.chdir(rP);
    await git.init();
  await gitcheckoutLocalBranch('master')
...
function getNewContent(body) {
  ...
  switch (body.variant) {
    ...
    case "Developer-Portfolio-1":
      ...
      repoPath = "Portfolio";
      buildPath = "dist";
      filePath = path.join(__dirname, "Portfolio", "src", "App.jsx");
...
app.patch('/user/site',async (req, res) => {
  ...
  try{
    const existingUser = await User.findOne({ userID: UserID });
    if(existingUser){
      const existingSite = existingUser.sites.find(site => site.SiteID === SiteID)
